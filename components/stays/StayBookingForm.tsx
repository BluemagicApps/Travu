...
export function StayBookingForm({ stay }: { stay: Stay }) {
  ...
  // Rooms implied by the offer total (so the server recomputes the same amount).
  const rooms = Math.max(1, Math.round(stay.totalPrice / (stay.pricePerNight * stay.nights)));
  ...
  return (
    <form onSubmit={submit} className="mt-4 space-y-5">
      ...
      >
        {loading ? "Processing…" : (
          <span>
            Pay <Money cents={stay.totalPrice} /> &amp; confirm
          </span>
        )}
      </button>
    </form>
