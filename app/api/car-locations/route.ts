...
/**
 * Pickup/drop-off suggestions for car rentals. Reuses the shared city/area index
 * and the flights airport dataset so "London (LHR – Heathrow)" appears — airports
 * are the most common car-rental pickup points.
 */
async function airportSuggestions(): Promise<LocationSuggestion[]> {
  try {
    ...
    return []; // DB unavailable — still serve city/area suggestions.
  }
}
